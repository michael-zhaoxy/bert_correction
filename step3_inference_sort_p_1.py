...
if __name__ == '__main__':
    ...
    f = open('./data/test_data/precision_test_priori.txt', 'r', encoding='utf-8')
    ...
    dict_trie = {}
    cache = set()
    f1 = open('./data/result/triePrecision.txt')
    for line in f1:
        line_ = line.strip().split('<:>')
        dict_trie[line_[0]] = line_[1]
    ...
    for line in (f):
        if line:
            ...
            # clean_text = re.sub('[^0-9a-zA-Z\u4e00-\u9fa5]', '', line)
            # prop_clean_text = getScore(clean_text)
            # if prop_clean_text > threshold(clean_text):
            #     threshold_cnt += 1
            #     continue
            ...
            if max_res in dict_trie:
                cache.add(max_res)
                result['纠正句子'].append(dict_trie[max_res])
            ...
            if max_res != original_text:
                ...
                print('{}<:>{}<:>{}<:>{}'.format(line, max_res, getScore(line), getScore(max_res)))
                # result_line.append('{}<:>{}'.format(line, max_res))
    ...
    for _ in dict_trie:
        ...
        for i in range(len(sq)):
            c1, c2 = max_res[i], sq[i]
            if c1 != c2:
                diff_cnt += 1


        prop_sq = getScore(sq)
        if (prop_sq > 1e-10 and prop_sq > max_prop * pow(10, diff_cnt)) or (prop_sq > max_prop * pow(10, diff_cnt * 2)):
            max_prop = prop_sq
            max_res = sq
        ...
        if max_res != _:
            print('{}<:>{}<:>{}<:>{}'.format(_, max_res, getScore(_), getScore(max_res)))
