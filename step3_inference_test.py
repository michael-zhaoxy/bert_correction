...
class Inference(object):
    def __init__(self, modelPath, mode='s'):
        self.char_count = 0
        self.top1_acc = 0
        self.top5_acc = 0
        self.sen_count = 0
        ...
        self.sen_wrong = 0
        self.mode = mode
        self.model = torch.load(modelPath).to(device).eval()
    ...
    def inference_batch(self, file_path):
        ...
        for line in (f):
            if line:
                ...
                src = line[0]
                label = line[1]
                result = self.inference_single(src, label)
                if result['纠正'] == label:
                    self.sen_acc += 1
                else:
                    if result['纠正'] == src:
                    ...
                    print('{}<:>{}<:>{}'.format(src, label, result['纠正']))


        print('句子正确个数：%s，句子未纠正个数：%s，句子错误个数：%s ， 句子总共个数：%s' %
